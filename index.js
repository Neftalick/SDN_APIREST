                        ...
                        if(err == null){
                            ...
                            if(result.length != 0){
                                if(SHA512(req.body.password) == result[0]["password"]){
                                    ...
                                    pool_usuarios_autenticados.query(`INSERT INTO usuarios_autenticados.dispositivo (dispositivo_ID, dispositivo_MAC, name) VALUES ("${req.body.mac}", "${req.body.mac}", 'computador')`,(err2,result2,fields2)=>{
                                        if(err2 == null){
                                            pool_usuarios_autenticados.query(`INSERT INTO usuarios_autenticados.usuario_autenticado (idUsuario_Autenticado,Dispositivo_dispositivo_MAC,diferenciador,switch_MAC,IP,Facultad_facultad_ID,Rol_idROL) VALUES ("${req.body.user}","${req.body.mac}","0","00:00:00:00:01:01","${req.body.ip}","${result[0]["Facultad_idFacultad"]}","${result[0]["Rol_idRol"]}")`,(err3,results3,fields3)=>{
                                                if(err3 == null){
                                                    res.json({
                                                        "status":"OK"
                                                    })
